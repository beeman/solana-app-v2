import { AppModal, ellipsify } from '@/components/app-layout.tsx'
...
import { useSolanaWallet, useSolanaWalletAddress } from '@/components/solana/data-access/solana-wallet-context.tsx'
...
export function AccountButtons({ address }: { address: Address }) {
  const { cluster } = useCluster()
  const isMainnet = cluster.network === 'mainnet-beta'
...
function ModalSend({ open, setOpen, address }: { setOpen: (open: boolean) => void; open: boolean; address: Address }) {
  ...
  if (!address || !wallet?.features.includes('solana:signAndSendTransaction')) {
    return <div>Wallet not connected or does not support signing transactions.</div>
  }
