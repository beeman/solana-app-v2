...
export interface Cluster {
  name: string
  endpoint: ClusterUrl
  endpointSubscriptions: ClusterUrl
...
// The endpoint used by the Solana Explorer (https://api.mainnet-beta.solana.com)
// does not allow access from arbitrary web apps due to CORS restrictions
// To use the mainnet-beta cluster, provide a custom endpoint
export const defaultClusters: Cluster[] = [
  {
    name: 'devnet',
    endpoint: devnet('https://api.devnet.solana.com'),
    endpointSubscriptions: devnet('wss://api.devnet.solana.com'),
    network: ClusterNetwork.Devnet,
  },
  {
    name: 'local',
    endpoint: 'http://localhost:8899',
    endpointSubscriptions: 'ws://localhost:8900',
  },
  {
    name: 'testnet',
    endpoint: testnet('https://api.testnet.solana.com'),
    endpointSubscriptions: testnet('wss://api.testnet.solana.com'),
...
export function ClusterProvider({ children }: { children: ReactNode }) {
  ...
  const value: ClusterProviderContext = {
    ...
    addCluster: (cluster: Cluster) => {
      if (clusters.find((item) => item.name.trim().toLowerCase() === cluster.name.trim().toLowerCase())) {
        toast({
          variant: 'destructive',
